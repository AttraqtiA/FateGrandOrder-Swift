class Caster: Servant { 
  ...
  override func attack(enemy: Servant) {
    ...
    override func noblePhantasm(enemy: Servant) {
      print("\n\(Colors.servantGold)\(self.name) uses Garden of Avalon! Heals 50% HP and deal 3.0x damage! \(Colors.reset)")
      self.currHP += self.maxHP * 0.5
      if self.currHP > self.maxHP {
        self.currHP = self.maxHP
      }
      enemy.minusHP(damage: self.attack * 3)
      
      self.NP_gauge = 0
    }

    // strong against Assassin
    // NP : Adds 500 Saint Quartz to your pocket
